...
<<<<<<< HEAD
	...
	def __set_user_agent(self):
		with open('user-agent.txt','r',encoding="utf-8")as fp:
			s = fp.read()
			url_li = json.loads(s, strict=False)
	...
	def shop_pass(self, pp):
		try:
			old_items = {}
			for i in self.items:
				old_items[i] = self.items[i]
			for ww in old_items:
				shoplink = "https:"+self.items[ww][1]
				# print(shoplink)
				...
				top_trl = r"https://shop%s.taobao.com"%shop_id
				end_url = top_trl + ajax_key
				# print(shop_id)
				txt2 = self.__req(end_url)
				if pp not in txt2:
					self.items.pop(ww)
				self.__test(r.text)
	...
	def data_output(self):
		if len(self.items) == 0:
			...
			with open("ww/ww%s.txt"%loc_time, "a", encoding="utf-8") as wf:
				for i in self.items:
					wf.write(i+"\t"+self.items[i][0]+"\t"+"https:"+self.items[i][1]+"\n")
...
if __name__ == "__main__":
    spiderMan = TaoSpider()
    spiderMan.craw()
>>>>>>> 0547564ca1f3a9c0e10f2f316c33440d5d2a3337
